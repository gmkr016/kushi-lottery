    ...
    public function index(Request $request, $pageSize = 10)
    {
        ...
        $user->getRelationValue('tickets')->each(function (Ticket $ticket) use (&$lists) {
            ...
            if (count($ticket->getRelation('lotteryNumbers'))) {
                $ticket->getRelationValue('lotteryNumbers')->each(function (LotteryNumber $lotteryNumber) use (&$lists) {
                    Arr::map(['first', 'second', 'third', 'fourth', 'fifth', 'sixth'], function ($key) use (&$lists, $lotteryNumber) {
                        $lists[$lotteryNumber['ticketId']]['numbers'][$key] = $lotteryNumber[$key];
                    });
                });
            }
        });
